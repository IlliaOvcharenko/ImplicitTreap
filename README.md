...
В начале теста находится команда clean, так что аккуратнее, чтоб не потерять данные случайно.
...
<название_файла_без_расширения> если тест лежит в папке /source/tests. Оба варианта аналогичны по функционалу,
второй просто быстрее Если тест загружен из графического режима (там есть кнопка "Add from file") отображается только дерамида с индексом 1.
